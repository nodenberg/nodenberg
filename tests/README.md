...
#### `npm test`
**役割**: TypeScript ビルドと主要 verify スクリプトをまとめて実行

**実行内容**:
1. `npm run build`
2. `tests/verify-print-settings.ts`
3. `tests/verify-section-table.ts`
4. `tests/verify-image-placeholder.ts`
5. `tests/verify-paging-e2e.ts`
...
#### `verify-print-layout.ts`
**役割**: `options.printLayout` によるExcel/PDF共通の印刷レイアウト設定を検証

**検証項目**:
1. **余白プリセット**
   - `marginPreset: "narrow"` が `pageMargins` に反映されること
   - `marginPreset` と `margins` の部分上書きが同時に効くこと

2. **ページ設定**
   - `paperSize`, `orientation`, `fit.width`, `fit.height` が `pageSetup` に反映されること
   - `fit` 指定時に `pageSetUpPr fitToPage="1"` が付与されること

3. **不正入力**
   - `margins: {}`、不明キー、負数、非数値、未知の用紙サイズ/向きが失敗すること
   - `recalculatePagination: false` が失敗すること

4. **PDF生成**
   - PDF生成が `printLayout` 適用済みのXLSX経路を通って成功すること
   - `template_section_02_with-tall-cell.xlsx` と `request_section_with-tall-cell.json` が存在する環境では、実fixtureでもPDF生成を検証

**実行方法**:
```bash
npx tsx tests/verify-print-layout.ts
```

`npm test` / `npm run test:verify` でも実行されます。
...
#### `verify-paging-e2e.ts`
**役割**: 単一 `Print_Area` + 手動 `rowBreaks` のページ割りを xlsx と PDF で検証

**検証項目**:
1. `Print_Area` がカンマ区切りではなく単一範囲であること
2. 手動 `rowBreaks` がレコード境界に揃うこと
3. 改ページ用の空白パディング行が挿入されないこと
4. LibreOffice で PDF 変換したページ数とページ先頭レコードが一致すること
...
```bash
npx tsx tests/verify-paging-e2e.ts
```
...
| `verify-print-layout.ts` | テストスクリプト | Excel/PDF共通の印刷レイアウト設定の検証 | `npx tsx tests/verify-print-layout.ts` |
...
3. `npx tsx tests/verify-print-layout.ts` でExcel/PDF共通の印刷レイアウト設定を検証
4. 出力ファイル（`output-test.xlsx`, `output-test.pdf`）を目視確認
